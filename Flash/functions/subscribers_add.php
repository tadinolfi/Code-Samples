		...
		switch (strtolower($action)) {
			...
			case 'save':
				...
				if (!$subscriber->ValidEmail($_POST['emailaddress'])) {
					$GLOBALS['Error'] = sprintf(GetLang('SubscriberAddFail_InvalidEmailAddress'), $_POST['emailaddress']);
				...
				if ($action == 'saveadd' || sizeof($user_lists) == 1) {
					...
					$session->Set('AddSubscriberMessage', $ListApi->Get('name'));
					?>
					<script>
		...
		if (!empty($customfields)) {
			...
			foreach ($customfields as $pos => $customfield_info) {
				...
				switch ($customfield_info['fieldtype']) {
					case 'date':
						$optionlist = '';
						$this->Display_CustomField($customfield_info);
					break;
