...
class Forms extends SendStudio_Functions
{
		...
		switch ($action) {
			...
			case 'finish':
				...
				if (isset($session_form['FormID']) && $session_form['FormID'] > 0) {
					...
					/* We create a md5 hash for added security */
					$salt = 'slr0xyoworldfoo';
					$formhash = md5($user->userid . $salt . rand(0, 500));
					$formapi->Set('formhash', $formhash);
	...
	*/
	function ShowConfirmationStep()
	{
		...
		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($GLOBALS['ConfirmPageHTML'], false, 260, 'confirmhtml');
		...
		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($htmlvalue, false, 260, 'confirmemail_html');
	...
	*/
	function ShowThanksStep()
	{
		...
		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($htmlvalue, false, 260, 'thanksemail_html');
		$GLOBALS['HTMLEditorName'] = 'thanksemail_html';
		$GLOBALS['EditorHTML'] = $this->ParseTemplate('Form_Editor_HTML', true, false);
		$GLOBALS['EditorText'] = $this->ParseTemplate('Form_Editor_Text', true, false);

		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($GLOBALS['ThanksPageHTML'], false, 260, 'thankspage_html');
	...
	*/
	function ShowFinalStep()
	{
		...
		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($GLOBALS['ErrorPageHTML'], false, 260, 'errorhtml');
	...
	*/
	function ShowFriendStep()
	{
		...
		$GLOBALS['HTMLContent'] = $this->GetHTMLEditor($htmlvalue, false, 260);
		$GLOBALS['EditorHTML'] = $this->ParseTemplate('Form_Editor_HTML', true, false);
		$GLOBALS['EditorText'] = $this->ParseTemplate('Form_Editor_Text', true, false);
		 ...
		 */
			...
			if (count($lists) == 0) {
				if (!$user->HasAccess('Lists', 'Create')) {
					...
					$GLOBALS['Message'] = $this->PrintSuccess('FormsNoLists', GetLang('FormsNoLists_NoAccess'));
				}
	...
	*/
	...
	function GetFormHTML($formapi = null)
	{
		...
		$GLOBALS['FormLink'] = SENDSTUDIO_APPLICATION_URL . '/forms/form.php?id=' . $formapi->formid . '&m=' . $formapi->formhash;
