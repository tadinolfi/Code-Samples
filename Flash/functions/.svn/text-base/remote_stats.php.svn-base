					...
					case 'step2':
						...
						foreach ($statids as $index=>$statid) {
							...
							if ($statstype == 'l') {
								...
								$session->Set('ListStatistics', $statid);
							}
									...
									case 'snapshot':
										switch ($statstype) {
											case 'l':
												...
												$data_url = SENDSTUDIO_APPLICATION_URL . '/admin/functions/stats_chart.php?Area=list&list='.$listid .'&graph=subscribersummary&'.session_name().'='.session_id();
											...
											case 'n':
												...
												$data_url = SENDSTUDIO_APPLICATION_URL . '/admin/stats_chart.php?Opens='.$summary['emailopens_unique'].'&Unopened='.$unopened.'&Bounced='.$total_bounces.'&'.session_name().'='.session_id();
											...
											case 'a':
												...
												$data_url = SENDSTUDIO_APPLICATION_URL . '/admin/stats_chart.php?Opens='.$summary['emailopens_unique'].'&Unopened='.$unopened.'&Bounced='.$total_bounces.'&Area=autoresponder&'.session_name().'='.session_id();
				...
				switch ($this->type) {
					case 'newsletter_opens':
						...
						foreach ($opens as $k => $opendetails) {
							$GLOBALS['EmailAddress'] = '<a href="' . SENDSTUDIO_APPLICATION_URL . '/admin/index.php?Page=Subscribers&Action=View&List=' . $opendetails['listid'] . '&id=' . $opendetails['subscriberid'] . '">' . htmlspecialchars($opendetails['emailaddress'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>';
					...
					if($chosen_link != 'b')
					{
						...
						foreach ($links as $k => $clickdetails) {
							$GLOBALS['EmailAddress'] = '<a href="' . SENDSTUDIO_APPLICATION_URL . '/admin/index.php?Page=Subscribers&Action=View&List=' . $clickdetails['listid'] . '&id=' . $clickdetails['subscriberid'] . '">' . htmlspecialchars($clickdetails['emailaddress'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>';
						...
						$all_links = $statsapi->GetUniqueLinks($statid, 0);
								...
								if (empty($all_links)) {
									...
									$GLOBALS['StatsLinkList'] = $all_links_list;
									$GLOBALS['PopularitySelected'] = 'SELECTED';
									$GLOBALS['StatsLinkDropDown'] = $this->ParseTemplate('Stats_Step3_Links_List', true, false);
									
									
									$click_list = '';
									// populate the rows with the link popularity overview data
									foreach ($all_links as $p => $linkinfo) {
									
									// we are re-using the existing placeholders, otherwise we would need to create a new page
									$GLOBALS['LinkID'] = $linkinfo['linkid'];
									// unique link
									$GLOBALS['UniqueLink'] = str_replace(array("'", '"'), "", $linkinfo['url']);
									// unique clicks
									$GLOBALS['UniqueClicks'] = $statsapi->GetUniqueClickRecipients($statid, '', $linkinfo['linkid']);
									// total clicks
									$GLOBALS['TotalClicks'] = $statsapi->GetClicks($statid, 0, 0, $linkinfo['linkid'], '', true);
								
	
									$click_list .= $this->ParseTemplate('Stats_Step3_Links_Overview_Row', true, false);
									}
									$GLOBALS['Stats_Step3_Links_List'] = $click_list;
									
								}
								echo $this->ParseTemplate('Stats_Step3_Links_Overview_Table');
						}
					...
					case 'newsletter_bounces':
						...
						foreach ($bounces as $k => $bouncedetails) {
							$GLOBALS['EmailAddress'] = '<a href=' . SENDSTUDIO_APPLICATION_URL . '/admin/index.php?Page=Subscribers&Action=View&List=' . $bouncedetails['listid'] . '&id=' . $bouncedetails['subscriberid'] . '">' . htmlspecialchars($bouncedetails['emailaddress'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>';
					...
					case 'newsletter_unsubscribes':
						...
						$token = $session->Get($token_request);
						...
						foreach ($unsubscribes as $k => $unsubdetails) {
							$GLOBALS['EmailAddress'] = '<a href=' . SENDSTUDIO_APPLICATION_URL . '/admin/index.php?Page=Subscribers&Action=View&List=' . $unsubdetails['listid'] . '&id=' . $unsubdetails['subscriberid'] . '>' . htmlspecialchars($unsubdetails['emailaddress'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>';
					...
					case 'forwards':
						...
						foreach ($forwards as $k => $forwarddetails) {
							...
							$GLOBALS['ForwardedBy'] = '<a href=' . SENDSTUDIO_APPLICATION_URL . '/admin/index.php?Page=Subscribers&Action=View&List=' . $forwarddetails['listid'] . '&id=' . $forwarddetails['subscriberid'] . '>' . htmlspecialchars($forwarddetails['forwardedby'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>';
