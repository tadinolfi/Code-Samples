...
class Stats extends SendStudio_Functions
{
					...
					case 'doselect':
						switch (strtolower($_REQUEST['SelectAction'])) {
							case 'export':
								...
								$http = SENDSTUDIO_TEMP_URL . "/download_csv.php?csv=" . urlencode($name);
	...
	*/
	function PrintNewsletterStats_Step2($statid=0)
	{
		...
		if ($sent_to != 0) {
                ...
                if ($sent_to == 0) { $GLOBALS['ClickThroughRate'] = "0%"; }
                else { $GLOBALS['ClickThroughRate'] = $this->FormatNumber($clicks / $sent_to * 100,2) . '%'; }
		...
		$data_url = 'stats_chart.php?Opens='.$summary['emailopens_unique'].'&Unopened='.$unopened.'&Bounced='.$total_bounces.'&'.session_name().'='.session_id();
	...
	function DisplayNewsletterOpens($statid, $summary=array(), $unique_only=false)
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $statid, 'unique_only' => $unique_only, 'calendar_restrictions' => $calendar_restrictions,
		  'summary' => $summary
		));
	...
	*/
	function DisplayNewsletterLinks($statid, $summary=array(), $chosen_link='a')
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $statid, 'chosen_link' => $chosen_link, 'calendar_restrictions' => $calendar_restrictions,
		  'summary' => $summary
		));
	...
	*/
	function DisplayNewsletterBounces($statid, $summary=array(), $chosen_bounce_type='')
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $statid, 'chosen_bounce_type' => $chosen_bounce_type, 'calendar_restrictions' => $calendar_restrictions,
		  'summary' => $summary
		));
	...
	*/
	function DisplayNewsletterUnsubscribes($statid, $summary=array())
	{
		...
		$GLOBALS['TotalComplaints'] = $this->FormatNumber($summary['total_complaints']);

		$most_unsubscribes = $statsapi->GetMostUnsubscribes($statid, $calendar_restrictions);
		
		$most_spamcomplaints = $statsapi->GetMostSpamComplaints($statid, $calendar_restrictions);
		...
		if (isset($most_spamcomplaints['mth'])) {
			$GLOBALS['MostSpamComplaints'] = $this->Months[$most_spamcomplaints['mth']] . ' ' . $most_spamcomplaints['yr'];
		}

		if (isset($most_spamcomplaints['hr'])) {
			$GLOBALS['MostSpamComplaints'] = $this->PrintDate(mktime($most_spamcomplaints['hr'], 1, 1, 1, 1, $now['year']), GetLang('Daily_Time_Display'));
		}

		if (isset($most_spamcomplaints['dow'])) {
			$pos = array_search($most_spamcomplaints['dow'], array_keys($this->days_of_week));
			// we need to add 1 hour here otherwise we get the wrong day from strtotime.
			$GLOBALS['MostSpamComplaints'] = $this->PrintDate(strtotime("last " . $this->days_of_week[$pos] . " +1 hour"), GetLang('Date_Display_Display'));
		}

		if (isset($most_spamcomplaints['dom'])) {
			$month = $now['mon'];
			// if the day-of-month is after "today", it's going to be for "last month" so adjust the month accordingly.
			if ($most_spamcomplaints['dom'] > $now['mday']) {
				$month = $now['mon'] - 1;
			}
			$GLOBALS['MostSpamComplaints'] = $this->PrintDate(mktime(0, 0, 1, $month, $most_spamcomplaints['dom'], $now['year']), GetLang('Date_Display_Display'));
		}
		...
		$session->Set($token,array(
		  'statid' => $statid, 'calendar_restrictions' => $calendar_restrictions, 'summary' => $summary
		));
	...
	*/
	function PrintUserStats_Step2($userid=0)
	{
		...
		$data_url = 'stats_chart.php?Heading=User&Opens='.$user_stats['unique_opens'].'&Unopened='.$unopened.'&Bounced='.$user_stats['total_bounces'].'&'.session_name().'='.session_id();
	...
	*/
	function CalculateCalendarRestrictions($calendarinfo=array(), $enddateonly=false)
	{
		...
		switch ($calendar_settings['DateType']) {
			...
			case 'lastmonth':
				$query = ' AND (%%TABLE%% >= ' . $this_month . ' AND %%TABLE%% < ' . $last_month . ')';
				$enddate = $this_month;
			break;
	...
	*/
	function DisplayChart($chartname='', $chart_area='', $statid=0, $type = 'pie', $settings = null, $listid=0)
	{
		...
		$data_url = 'stats_chart.php?graph=' . urlencode(strtolower($chartname)) . '&Area='.urlencode(strtolower($chart_area)) . '&statid=' . (int)$statid . '&' . session_name().'='.session_id();
	...
	*/
	function PrintAutoresponderStats_Step2($autoresponderid=0)
	{
		...
		$data_url = 'stats_chart.php?Opens='.$summary['emailopens_unique'].'&Unopened='.$unopened.'&Bounced='.$total_bounces.'&Area=autoresponder&'.session_name().'='.session_id();
	...
	*/
	function DisplayAutoresponderOpens($statid, $autoresponderid, $summary=array(), $only_unique=false)
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $statid, 'calendar_restrictions' => $calendar_restrictions,'unique_only' => $only_unique,
		  'summary' => $summary
		));
	...
	*/
	function DisplayAutoresponderLinks($statid, $autoresponderid, $summary=array(), $chosen_link='a')
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $statid, 'calendar_restrictions' => $calendar_restrictions,'chosen_link' => $chosen_link,
		  'summary' => $summary
		));
	...
	*/
	function DisplayAutoresponderBounces($statid, $autoresponderid, $summary=array(), $chosen_bounce_type='')
	{
		...
		$session->Set($token,array(
		  'statid' => $statid, 'calendar_restrictions' => $calendar_restrictions,'chosen_bounce_type' => $chosen_bounce_type,
		  'summary' => $summary
		));
	...
	*/
	function PrintListStats_Step2($listid=0)
	{
		...
		$data_url = 'stats_chart.php?Area=list&list='.$listid .'&graph=subscribersummary&'.session_name().'='.session_id();
	...
	*/
	function DisplayListOpens($listid=0, $summary, $unique_only=false)
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $summary['statids'], 'calendar_restrictions' => $calendar_restrictions, 'unique_only' => $unique_only, 'summary' => $summary
		));
	...
	*/
	function DisplayListLinks($listid, $summary=array(), $chosen_link='a')
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $summary['statids'], 'calendar_restrictions' => $calendar_restrictions, 'chosen_link' => $chosen_link, 'summary' => $summary
		));
	...
	*/
	function DisplayListBounces($listid, $summary, $chosen_bounce_type)
	{
		...
		*/
		...
		$session->Set($token,array(
		  'statid' => $summary['statids'], 'calendar_restrictions' => $calendar_restrictions, 'chosen_bounce_type' => $chosen_bounce_type, 'summary' => $summary
		));
	...
	*/
	...
	// --------------------------------------------------------------------------------
		...
		function TriggerEmailsStats($subaction)
		{
			// ----- Check user access
				$tempUser = GetUser();
				if (!$tempUser->HasAccess('Statistics', 'TriggerEmails')) {
					$this->PrintNewsletterStats_Step1();
					return;
				}
				unset($tempUser);
		...
		function TriggerEmailsStats_View()
		{
			...
			// ----- Calculate some common variables for the record
				...
				if ($record['processed_totalsent'] != 0) {
					...
					$session->Set($page['session_token'], array(
			...
			// ----- Tab 1: Snapshot
				...
				$tabs['snapshot']['summary_chart'] = InsertChart(
					'pie',
					'stats_chart.php?Opens=' . $record['emailopens_unique'] . '&Unopened=' . $record['processed_unopened'] . '&Bounced=' . $record['processed_totalbounced'] . '&Area=TriggerEmails&'. session_name() . '=' . session_id(),
