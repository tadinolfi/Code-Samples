...
class Autoresponders extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		switch ($action) {
			case 'pause':
				$this->_pauseAutoresponder();
			break;

			case 'resume':
				$this->_resumeAutoresponder();
			break;

			case 'viewcompatibility':
				$session = &GetSession();
				$auto_info = $session->Get('Autoresponders');
			...
			case 'view':
				...
				IEM::logUserActivity($_SERVER['REQUEST_URI'], 'images/autoresponders_view.gif', $autoresponderapi->name);
	...
	*/
	...
	/**
	 * _pauseAutoresponder
	 * Pause autoresponder(s)
	 * @return Void Does not return anything
	 */
	function _pauseAutoresponder()
	{
		/**
		 * Local variables
		 */
			$autoresponderID = intval($this->_getGETRequest('id', 0));
			$listID = intval($this->_getGETRequest('listid', 0));
	...
	/**
	 * _resumeAutoresponder
	 * Resume autoresponder(s)
	 * @return Void Does not return anything
	 */
	function _resumeAutoresponder()
	{
