...
require(dirname(__FILE__) . '/folders.php');
...
class Lists extends SendStudio_Functions
{
	 ...
	 *
	 * @param Array $param User and Session objects needed by this function.
	 *
	...
	private function CopyList($param)
	{
		...
		if (!$result) {
			...
			$param['session']->Set('UserDetails', $param['user']);
			$param['session']->Remove('UserLists');
	...
	private function ManageLists($param)
	{
		...
		foreach ($mylists as $pos => $listinfo) {
			...
			$GLOBALS['SubscriberAction'] = '';
			$GLOBALS['ListAction'] = '';

			$GLOBALS['ListID'] = $listinfo['listid'];
			$GLOBALS['SubscriberAction'] .= $this->ParseTemplate('Lists_Manage_ViewSubscribersLink', true);
			
			$GLOBALS['SubscriberAction'] .= $this->ParseTemplate('Lists_Manage_Searchlink', true);

			if ($user->HasAccess('Subscribers', 'Add')) {
				$GLOBALS['AddSubscriberListID'] = $listinfo['listid'];
				$GLOBALS['SubscriberAction'] .= $this->ParseTemplate('Lists_Manage_AddSubscriberLink', true, false);
			}
	...
	private function EditList($param)
	{
		...
		if ($user->HasAccess('Lists', 'BounceSettings')) {
			...
			$GLOBALS['Bounce_AgreeDelete'] = ($list->agreedelete == 1)? ' CHECKED' : '';
			$GLOBALS['Bounce_AgreeDeleteAll'] = ($list->agreedeleteall == 1)? ' CHECKED' : '';
		} else {
			$GLOBALS['ShowBounceInfo'] = 'none';
			$GLOBALS['DisplayExtraMailSettings'] = 'none';

			$GLOBALS['BounceEmail'] = 'summy@email.com';
		}
	...
	private function UpdateList($param)
	{
		 ...
		 */
			if ($param['user']->HasAccess('Lists', 'BounceSettings')) {
				...
				$list->imapaccount = (isset($_POST['bounce_imap'])) ? 1 : 0;
				...
				$list->agreedelete = (isset($_POST['bounce_agreedelete'])) ? 1 : 0;
		 ...
		 */
			...
			if ($user->HasAccess('Lists', 'BounceSettings')) {
				...
				if (SENDSTUDIO_BOUNCE_AGREEDELETE == 1) {
					...
					$GLOBALS['Bounce_AgreeDelete'] = ' CHECKED';
	...
	private function ChangeList($param)
	{
		...
		if ($subscribers_deleted_failure > 0) {
			$message = GetLang('ListsDeleteAllSubscribersFailure');
			if ($subscribers_deleted_failure == 1) {
				$message = GetLang('ListDeleteAllSubscribersFailure');
			}
			FlashMessage($message, SS_FLASH_MSG_ERROR);
		}
	 ...
	 *
	 * @param Array $param The Session object needed by this function.
	 *
