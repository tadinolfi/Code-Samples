...
class Subscribers extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		switch ($action) {
			...
			case 'view':
				require(dirname(__FILE__) . '/subscribers_view.php');
			...
			case 'event':
				require(dirname(__FILE__) . '/subscribers_event.php');
	...
	*/
	function ChooseList($action='Manage', $subaction=null)
	{
		...
		if ($user->HasAccess('Segments', 'View')) {
			$selectSegment = '';
	...
	*/
	function SetupGoogleCalendar()
	{
		...
		$eventGoogleUse = $session->Get('eventGoogleUse');
