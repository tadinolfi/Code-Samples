	...
	*/
	function ManageSchedules()
	{
		...
		foreach ($jobs as $p => $details) {
			...
			{
				$GLOBALS['TipName'] = 'PauseTip_' . $p;
				$GLOBALS['Status'] = $this->ParseTemplate('Schedule_Pause_Tip', true, false);
			} else {
			...
			if ($details['jobstatus'] != 'i' && $details['jobstatus'] != 'w' && $details['jobstatus'] != 'p') {
			...
			if($details['jobstatus'] == 'p' && ($user->UserAdmin() || $user->Admin()))
			{
				$action .= $this->ParseTemplate('Schedule_Manage_Row_AdminDeleteLink', true, false);
			}
	...
	*/
	...
	function DeleteJobs($jobids=array())
	{
		...
		foreach ($jobids as $p => $jobid) {
			...
			} elseif ($statid == 0 && $jobinfo['jobstatus'] != 'c') {
				if ($user->RefundEmails($jobinfo['jobdetails']['SendSize'], $jobinfo['ownerid']))
				{
					echo "Credits Refunded Sucessfully";
				}
			}
