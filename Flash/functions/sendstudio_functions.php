...
class SendStudio_Functions
{
	...
	*/
	function PrintHeader($PopupWindow=false, $LoadLanguageFile=true, $PrintHeader=true)
	{
		...
		// Check if a clientId is specified
		...
		if($clientid > 0 && isset($_SESSION['agencylogged']))
		{
		...
		} elseif (isset($_SESSION['agencylogged']) && $clientid == '') {
			$user = &GetUser($_SESSION['agencylogged']);
		...
		if(isset($user) && $user->userid > 0){
			$GLOBALS['ClientList'] = $user->GetClientList($clientid);
		}
		if(isset($_SESSION['agencylogged']) && $_SESSION['agencylogged'] != "")
		{
			$GLOBALS['ShowClientList'] = 'block';
		}else{
			$GLOBALS['ShowClientList'] = 'none';
		}
		if((isset($_GET['Action']) && strtolower($_GET['Action']) != 'manage') || isset($_GET['SubAction']))
		{
			$GLOBALS['DisableClientList'] = 'DISABLED';
		}
	...
	*/
	static function GenerateTextMenuLinks()
	{
		...
		if ($lke && (!isset($_GET['Page']) || strtolower($_GET['Page']) == 'settings')) {
			...
			$textlinks .= '<a href="JavaScript:LaunchHelp();" class="MenuText" title="' . GetLang('Menu_Help_Description') . '">' . GetLang('ShowHelp') . '</a>';
			return $textlinks;
		}
		...
		if($user->UserAdmin())
		{
		$links['credithistory'] = array (
			array (
			'text' => GetLang('CreditHistory'),
			'link' => 'index.php?Page=Credits',
			'show' => true,
			'description' => GetLang('Menu_Credit_History_Description'),
		));
		}
		
		if($user->UserAdmin())
		{
		$links['bounceadmin'] = array (
			array (
			'text' => GetLang('BounceAdmin'),
			'link' => 'index.php?Page=BounceAdmin',
			'show' => true,
			'description' => GetLang('Menu_Bounce_Admin_Description'),
		));
		
		$links['seedlist'] = array (
			array (
			'text' => GetLang('SeedList'),
			'link' => 'index.php?Page=Seedlist',
			'show' => true,
			'description' => GetLang('Menu_SeedList_Description'),
		));
		}
		
		
		if (!$user->UserAdmin() && $user->EditOwnSettings()) {
		...
		if ($user->UserAdmin()) {
			$links['users'] = array (
				array (
					...
					'show' => true,
					'description' => GetLang('Menu_Users_Description'),
				)
			);
		}
		...
		foreach ($links as $link_area => $sublinks) {
			...
			if (!$has_submenu) {
				...
				$textlinks .= '<a ' . (array_key_exists('target', $link_details) ? $link_details['target'] : '') . ' href="' . $link_details['link'] . '" class="MenuText" title="' . htmlspecialchars($link_details['description'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '">' . htmlspecialchars($link_details['text'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '</a>|';
				continue;
			}
			...
			foreach ($sublinks as $link_details) {
				$sublink .= '<a href="' . $link_details['link'] . '">' . $link_details['text'] . '</a>';
			}
			$sublink .= '</ul></div>|';
	...
	static function GenerateMenuLinks()
	{
		...
		$menuItems = array (
			'contactlist_button' => array (
				...
				array (
					'text' => GetLang('Menu_Segments_Create'),
					'link' => 'index.php?Page=Segment&Action=Create',
					'show' => $user->HasAccess('Segments', 'Create'),
					'description' => GetLang('Menu_Segment_Create_Description'),
					'image' => 'menuarrow.jpg'
				),
				array (
			...
			'newsletter_button' => array (
				...
				//see addons/splittest/splittest.php for splittest menu item addon
				array (
					'text' => 'Create Split Test',
					'link' => 'index.php?Page=Addons&Addon=splittest&Action=create',
					'show' => $user->HasAccess('splittest', 'create'),
					'description' => '',
					'image' => 'menuarrow.jpg'
				)
			...
			'autoresponder_button' => array (
				...
				array (
					...
					'show' => ($user->HasAccess('TriggerEmails')),
					'description' => GetLang('Menu_TriggerEmails_Description'),
					'image' => 'menuarrow.jpg'
				),
				array (
					'text' => GetLang('Menu_CreateTriggerEmails'),
					'link' => 'index.php?Page=TriggerEmails&Action=Create',
					'show' => ($user->HasAccess('TriggerEmails')),
					'description' => GetLang('Menu_TriggerEmails_Create_Description'),
					'image' => 'menuarrow.jpg'
				)
			),
			'templates_button' => array(
			   array (
					'text' => GetLang('Menu_Templates_Manage'),
					'link' => 'index.php?Page=Templates&amp;Action=Manage',
					'show' => $user->HasAccess('Templates', 'Manage'),
			...
			'statistics_button' => array (
				...
				array (
					...
					'show' => ($user->HasAccess('Statistics', 'TriggerEmails')),
					...
					*/
					...
					if ($first) {
						...
						/*if ($menu_selected) {
							...
							// if we find the base directory in the image path, then it's a full path name (from an addon).
							if (strpos($image, SENDSTUDIO_BASE_DIRECTORY) !== false) {
								$filename = $image . '_selected.gif';
								$image_url = str_replace(SENDSTUDIO_BASE_DIRECTORY, SENDSTUDIO_APPLICATION_URL, $filename);
							} else {
								$filename = $imagesDir. DIRECTORY_SEPARATOR . $image . '_selected.gif';
								$image_ext = '_selected.gif';
							}
							$image .= '_selected';
						} else {*/
						...
						$menu .= '<a class="mainNav" href="'.$sub['link'].'"' . $target . '>';
					...
					if (count($link) > 1) {
						...
						if($sub['text'] == 'Create a Contact List' || 
						   $sub['text'] == 'Create a Segment' ||
						   $sub['text'] == 'Remove Contacts' ||
						   $sub['text'] == 'View Scheduled Email Queue' ||
						   $sub['text'] == 'Create an Autoresponder' ||
						   $sub['text'] == 'Create Render Test' ||
						   $sub['text'] == 'Create a Custom Template' )
							$menu .= ' style="background-image:url(images/menuhr.jpg);background-repeat:no-repeat;background-position:left bottom;padding-bottom:2px;"';
						$menu .= '><a style="background-image: url('.$icon.');" class="menu_mnuStats" href="'.$sub['link'].'" '.$target.'>'.$sub['text'].'</a></li>'."\n";
					}
				}
				if ($shown) {
					if (count($link) > 1) {
	...
	*/
	function LoadLanguageFile($languagefile=null)
	{
		...
		if (!is_dir(SENDSTUDIO_LANGUAGE_DIRECTORY . "/{$users_language}")) {
			$users_language = 'default';
		}

		$languagefile = strtolower($languagefile);
		$langfile = SENDSTUDIO_LANGUAGE_DIRECTORY . '/' . $users_language . '/' . $languagefile . '.php';
	...
	*/
	...
	{
		$page = isset($_GET['Page']) ? strtolower($_GET['Page']) : 'unknown';
	...
	*/
	function GetHTMLEditor($value='', $id=false, $height=580, $name='myDevEditControl')
	{
		$session = &GetSession();
		$user = $session->Get('UserDetails');

		if (!$user->Get('usewysiwyg')) {
			$GLOBALS['HTMLContent'] = htmlentities($value, ENT_QUOTES, SENDSTUDIO_CHARSET);
			$GLOBALS['Name'] = $name .= '_html';
			return $this->ParseTemplate('editor_html_no_wysiwyg', true, false);
		}

		$type = ucwords(get_class($this));

		$deveditpath = $this->GetAppPath() . '/admin/de';
		...
		*/
		...
		$max_dims['width'] = SENDSTUDIO_MAX_IMAGEWIDTH;
		$max_dims['height'] = SENDSTUDIO_MAX_IMAGEHEIGHT;
		$max_dims = array_map('intval', $max_dims);
		$myDE->SetImageUploadSize($max_dims['width'], $max_dims['height']);
		...
		if ((is_numeric(strpos($value, 'name="tid"'))) || (is_numeric(strpos($value, "name='tid'")))) {
			$myDE->AddToolbarColorButton("themebutton","Change Theme Color","theme","switchTheme","Simple",1,1);
		}
		$myDE->SetValue($value);
		...
		if (check($name, $ROOTDIR1)) {
			$myDE->AddCustomLink(GetLang('Link_MailingListArchives'), "%%mailinglistarchive%%");
		...
		if (!empty($modify_forms)) {
			foreach ($modify_forms as $p => $formdetails) {
				$myDE->AddCustomLink(htmlspecialchars($formdetails['name'], ENT_QUOTES, SENDSTUDIO_CHARSET), "%%modifydetails_" . $formdetails['formid'] . "%%");
			}
		}
		...
		if (!empty($sendfriend_forms)) {
			foreach ($sendfriend_forms as $p => $formdetails) {
				$myDE->AddCustomLink(htmlspecialchars($formdetails['name'], ENT_QUOTES, SENDSTUDIO_CHARSET), "%%sendfriend_" . $formdetails['formid'] . "%%");
			}
		}

		/**
		 * Getting custom button from language variable,
		 * and make sure they do not contains comma (,)
		 */
		$customToolbar = array(
			'CustomField' => str_replace(',', '', GetLang('CustomFieldsInsert_Editor')),
			'InsertImage' => str_replace(',', '', GetLang('ImageInsert_Editor'))
		);

		$myDE->AddToolbarButton($customToolbar['CustomField'], 97, 20, SENDSTUDIO_IMAGE_URL . "/customfields_toolbaricon.gif", "ShowCustomFields('html', '" . $name . "')");

		$myDE->AddToolbarButton($customToolbar['InsertImage'], 57, 20, SENDSTUDIO_IMAGE_URL . "/image_toolbaricon.gif", $name . ".commands.getCommand('Image').execute();");

		// Setup the toolbars
		$rows = array (
			"themebutton,Fullscreen,Undo,Redo,Paste,Findreplace,Spellcheck,-,Bold,Underline,Italic,-,OrderedList,UnorderedList,-,JustifyLeft,JustifyCenter,JustifyRight,JustifyFull,-,CreateLink,Anchor,Help",
			'Fontname,Fontsize,-,Fontcolor,Highlight,-,' . $customToolbar['InsertImage'] . ',-,Table,-,' . $customToolbar['CustomField']
		);
		$myDE->AddToolbar("Simple", implode(",|,", $rows));

		$rows = array (
			"themebutton,Fullscreen,Undo,Redo,Paste,Findreplace,Spellcheck,Bold,Underline,Italic,Strikthrough,-,OrderedList,UnorderedList,-,JustifyLeft,JustifyCenter,JustifyRight,JustifyFull,-,Help,",
			'Spellcheck,-,RemoveFormat,-,Indent,Outdent,-,SubScript,SuperScript,CreateLink,CreateEmailLink,Anchor,-,' . $customToolbar['InsertImage'] . ',-,Table,Form,Pageproperties,-,' . $customToolbar['CustomField'],
			"Fontname,Fontsize,Formatblock,Fontcolor,Highlight,HR,Insertchars,Showborders, themebutton",
		);
		$myDE->AddToolbar("Complete", implode(",|,", $rows));

		// Change the width of the editor
		$GLOBALS["EditorWidth"] = "95%";

		//Display the DevEdit control. This *MUST* be called between <form> and </form> tags
		$myDE->ShowControl($GLOBALS["EditorWidth"],$height, $ROOTDIR2);
	...
	*/
	function PreviewWindow($details=array(), $showBroken = false, $sync = false)
	{
		...
		$set_sync = '';
		if ($sync) {
			$windows = $session->Get('PreviewWindowHash');
			if (!is_array($windows)) {
				$windows = array();
			}
			$details['modified'] = time();
			$windows[$sync] = $details;
			$session->Set('PreviewWindowHash', $windows);
	...
	*/
	function SendPreview()
	{
		...
		*/
		...
		foreach ($preview_email_separate as $recipient) {
...
*/
		...
		//$output = '<timepicker id="sendtime" displaySeconds="false" displayAMPM="true" leadZero="true" value="' . $currentTime . '" enforceDefaultValue="false" class="timePicker"></timepicker>';

		// Changed from a timepicker to multiple select boxes - changed by Mitch during Alpha testing because
		// the timepicker was too hard to position the way I wanted it to be and it's too fiddly
	...
	*/
	 ...
	 */
	function CheckContentForSpamDisplay()
	{
		$this->ParseTemplate('Spam_Check_Display');
	}
	...
	function CheckContentForSpam()
	{
		...
		$rule_types = $spam_api->Get('ruletypes');

		$html_broken_rules = array();
		$text_broken_rules = array();

		$html_score = 0;
		$spam_api->TotalSpamRating = Array();
		$spam_api->BrokenRules = Array();
		if ($html) {
			foreach ($rule_types as $ruletype) {
				$spam_api->Process($html, $ruletype);
				$html_score += $spam_api->GetSpamRating($ruletype);
			}
			$html_broken_rules = $spam_api->GetBrokenRules();
		}
		...
		if ($text !== false) {
			foreach ($rule_types as $ruletype) {
				$spam_api->Process($text, $ruletype);
				$text_score += $spam_api->GetSpamRating($ruletype);
			}
			$text_broken_rules = $spam_api->GetBrokenRules();
		} else {
			/**
			 * Spam assasin have a rule where emails must always contains "text" part
			 */
			$text_score += 1.2;
			$text_broken_rules = array(
				'spamassasin' => array(
		...
		/**
		 * Spam assasin have a rule where HTML and TEXT contents should be similar
		 */
			if ($html !== false && $text !== false) {
				if (similar_text(strip_tags($html), $text) < 0.5) {
					$temp = array(
						'HTML and text parts are different',
						'1.5'
					);

					if (!array_key_exists('spamassasin', $text_broken_rules)) {
						$text_broken_rules['spamassasin'] = array($temp);
					} else {
						$text_broken_rules['spamassasin'][] = $temp;
					}

					if (!array_key_exists('spamassasin', $html_broken_rules)) {
						$html_broken_rules['spamassasin'] = array($temp);
					} else {
						$html_broken_rules['spamassasin'][] = $temp;
					}
		...
		$broken_rules_area = array('html', 'text');

		foreach ($broken_rules_area as $c => $broken_area) {
			if ($c > 0) {
				$this->ParseTemplate('Spam_Check_Row_Empty');
			}

			$broken_rules = $broken_area . '_broken_rules';
			if (empty(${$broken_rules})) {
				if (${$broken_area} !== false) {
					$GLOBALS['Spam_Heading'] = GetLang('Spam_Heading_intro_notspam_' . $broken_area);
					$this->ParseTemplate('Spam_Check_Row_Not_Broken');
				}
				continue;
			}
			$GLOBALS['Spam_Heading'] = GetLang('Spam_Heading_intro_' . $broken_area);
			$this->ParseTemplate('Spam_Check_SubHeader');

			$rule_types = array_keys($$broken_rules);
			foreach ($rule_types as $ruletype) {
				foreach (${$broken_rules}[$ruletype] as $details) {
					$GLOBALS['RuleBroken'] = $details[0];
					$GLOBALS['RuleScore'] = $this->FormatNumber($details[1], 1);
					$this->ParseTemplate('Spam_Check_Row');
				}
			}

			$ratings = $spam_api->Get('Spam_Rating');

			$total_score = ${$broken_area.'_score'};

			if ($total_score < $ratings['notspam']) {
				$display_style = 'notspam';
			} else {
				if ($total_score < $ratings['alert']) {
					$display_style = 'alert';
				} else {
					$display_style = 'spam';
				}
			...
			$total_spam_rating = $spam_api->Get('SpamFilterLimit');

			$score_percent = number_format((($total_score / $total_spam_rating) * 100), 0);

			if ($score_percent > 100) {
				$score_percent = 100;
			}

			$GLOBALS['Spam_Percentage'] = $score_percent;

			$GLOBALS['Spam_Display_Style'] = $display_style;

			$GLOBALS['Spam_Rating_Message'] = sprintf(GetLang('Spam_Rating_Message'), $total_score, $total_spam_rating, GetLang('Spam_Rating_' . $display_style));

			$this->ParseTemplate('Spam_Check_SubFooter');
		}
	}
		...
		if ($page != 'send' && $page != 'schedule') {
			...
			*/
			...
			if ($approve_job) {
				...
				$send_size = $session->Get('JobSendSize');

				if ($send_size) {
					$new_max = $user->Get('maxemails') + $send_size;
					if ($new_max < 0) {
						$new_max = 0;
					}
	...
	*/
	function GoogleCalendarAdd($data,$test_auth = false)
	{
		if (!class_exists('GoogleCalendar')) {
			require(SENDSTUDIO_LIB_DIRECTORY . "/googlecalendar/googlecalendar.php");
		}
