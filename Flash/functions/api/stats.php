...
class Stats_API extends API
{
	...
	function Delete($statids=array(), $statstype='n')
	{
		 ...
		 */
			...
			if ($convertedStatType != '') {
				$tempContinue = false;
				if (!class_exists('module_TrackerFactory')) {
	...
	*/
	...
	function GetGraphData($statids=array(), $stats_type=false, $restrictions='', $chart_type='', $listid=0)
	{
		...
		} else if ($chart_type == 'unsubscribechart') {
	...
	*/
	function CalculateGroupBy($stats_type=false, $fieldname='')
	{
		...
		if (SENDSTUDIO_DATABASE_TYPE == 'mysql') {
			$timestamp = "CONVERT_TZ(FROM_UNIXTIME($fieldname + $server_tz),'SYSTEM','$user_tz')";
	...
	*/
	function GetUniqueClickRecipients($statids=array(), $calendar_restrictions='',$linkid = 'a')
	{
		...
		$query = "
			SELECT COUNT(DISTINCT lc.linkid) AS count FROM " . SENDSTUDIO_TABLEPREFIX . "stats_linkclicks lc
	...
	*/
	function GetClicks($statids=array(), $start=0, $perpage=10, $linkid='a', $calendar_restrictions='', $count_only=false, $order_by = 'clicktime', $order_dir = 'DESC')
	{
		...
		$query = "SELECT l.emailaddress, clicktime, clickip, url, l.listid, l.subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "stats_linkclicks lc, " . SENDSTUDIO_TABLEPREFIX . "links ml WHERE ml.linkid=lc.linkid AND l.subscriberid=lc.subscriberid AND lc.statid IN(" . implode(',', $statids) . ") " . $calendar_restrictions;
	...
	*/
	function GetBounces($statids=array(), $start=0, $perpage=10, $bounce_type=false, $calendar_restrictions='', $count_only=false,$order_by = 'bouncetime', $order_dir = 'DESC')
	{
		...
		$query = "SELECT l.emailaddress, bouncetime, bouncetype, bouncerule, l.listid, l.subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "list_subscriber_bounces b WHERE l.subscriberid=b.subscriberid AND b.statid IN(" . implode(',', $statids) . ") " . $calendar_restrictions;
	...
	*/
	function GetForwards($statids=array(), $start=0, $perpage=10, $calendar_restrictions='', $count_only=false, $new_signups=false,$order_by = 'forwardtime', $order_dir = 'DESC')
	{
		...
		$query = "SELECT l.emailaddress AS forwardedby, forwardtime, forwardip, f.emailaddress AS forwardedto, subscribed, l.listid, l.subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "stats_emailforwards f WHERE l.subscriberid=f.subscriberid AND f.statid IN(" . implode(',', $statids) . ") " . $calendar_restrictions;
	...
	*/
	function GetUnsubscribes($statids=array(), $start=0, $perpage=10, $calendar_restrictions='', $count_only=false,$order_by = 'unsubscribetime', $order_dir = 'DESC', $listid=0)
	{
		...
		$query = "SELECT l.emailaddress, unsubscribetime, unsubscribeip, l.listid, l.subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "list_subscribers_unsubscribe lsu WHERE l.subscriberid=lsu.subscriberid AND ";
		if ($listid > 0) {
			$query .= " lsu.listid='" . $listid . "'";
		} else {
			$query .= " lsu.statid IN (" . implode(',', $statids) . ") ";
	...
	*/
	function GetOpens($statids=array(), $start=0, $perpage=10, $only_unique=false, $calendar_restrictions='', $count_only=false, $order_by = 'opentime', $order_dir = 'DESC')
	{
		...
		if (!$only_unique) {
			...
			$query = "SELECT l.emailaddress, opentime, openip, opentype, l.subscriberid, l.listid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "stats_emailopens o WHERE l.subscriberid=o.subscriberid AND o.statid IN(" . implode(',', $statids) . ") " . $calendar_restrictions . " ORDER BY $order_by $order_dir ";
		} else {
			if ($count_only) {
				/**
			...
			if (SENDSTUDIO_DATABASE_TYPE == 'mysql') {
				$query = "SELECT l.emailaddress, opentime, openip, opentype, l.subscriberid, l.listid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l, " . SENDSTUDIO_TABLEPREFIX . "stats_emailopens o WHERE l.subscriberid=o.subscriberid AND o.statid IN(" . implode(',', $statids) . ") " . $calendar_restrictions . " GROUP BY l.emailaddress ORDER BY $order_by $order_dir ";
			} else {
	...
	*/
	function MarkNewsletterFinished($statids=array(), $original_queuesizes=array())
	{
		 ...
		 */
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "stats_newsletters SET finishtime='" . (int)$endtime . "' WHERE statid IN (" . implode(',', $statids) . ")";
		$this->Db->Query($query);
		 ...
		 */
		$query = "SELECT (multipartrecipients + htmlrecipients + textrecipients) AS totalrecipients, queueid, sentby, statid FROM " . SENDSTUDIO_TABLEPREFIX . "stats_newsletters WHERE statid IN (" . implode(',', $statids) . ")";
		$result = $this->Db->Query($query);
		...
		while ($row = $this->Db->Fetch($result)) {
			...
			$total_sent = (int)$row['totalrecipients'];

			$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "stats_users SET queuesize=" . $total_sent . " WHERE statid=" . (int)$row['statid'];
		...
		/**
		 * If the original queue size is larger than the total we sent to,
		 * give them back the difference.
		 */
		if ($original_queuesize > $total_recipients) {
			$credits = $original_queuesize - $total_recipients;
			$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "users SET maxemails = maxemails + " . $credits . " WHERE userid='" . (int)$userid . "' AND unlimitedmaxemails='0'";
			$this->Db->Query($query);
		} else {
			/**
			 * If we sent more than the original queue size,
			 * then make sure we take those extra credits away from the user.
			 */
			$difference = $total_recipients - $original_queuesize;
			if ($difference > 0) {
				$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "users SET maxemails = maxemails - " . $difference . " WHERE userid='" . (int)$userid . "' AND unlimitedmaxemails='0'";
				$this->Db->Query($query);
			}
		}
	...
	*/
	function GetNewsletterSummary($statid=0, $skip_subareas=false, $subarea_limit=10)
	{
		...
		$query = "SELECT u.subscriberid, l.emailaddress, unsubscribetime, unsubscribeip FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers_unsubscribe u, " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l WHERE l.subscriberid=u.subscriberid AND statid='" . $statid . "' AND u.complaint='1' ORDER BY unsubscribetime DESC LIMIT " . $subarea_limit;
		$result = $this->Db->Query($query);
		$total_complaints = $this->Db->CountResult($result);
		...
		$query = "SELECT lc.clickid, ls.emailaddress, lc.clicktime, l.url FROM " . SENDSTUDIO_TABLEPREFIX . "links l, " . SENDSTUDIO_TABLEPREFIX . "list_subscribers ls, " . SENDSTUDIO_TABLEPREFIX . "stats_linkclicks lc WHERE l.subscriberid=lc.subscriberid AND l.linkid=lc.linkid AND lc.statid='" . $statid . "' ORDER BY clicktime DESC LIMIT " . $subarea_limit;
		$result = $this->Db->Query($query);
	...
	/**
	 * GetMostSpamComplaints
	 * Returns the month of in which the most spam complaints were logged
	 *
	 * @param Array $statids
	 * @param String $restrictions
	 * @return Mixed returns an array resource
	 */
	
	function GetMostSpamComplaints($statids=array(), $restrictions='')
	{
		if (!is_array($statids)) {
			$statids = array($statids);
		}
		...
		$qry .= $this->CalculateGroupBy($this->calendar_type, 'unsubscribetime');
		$qry .= " FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers_unsubscribe";
		$qry .= " WHERE statid IN(" . implode(',', $statids) . ") AND complaint='1'";
		if ($restrictions) {
			$qry .= $restrictions;
		}
	...
	*/
	function GetUniqueLinks($statids=array(),$listid = 0)
	{
		...
		$query = "SELECT l.linkid AS linkid, url";

		if ($listid > 0) {
			$query .= ", ls.listid as listid";
		}

		$query .= " FROM " . SENDSTUDIO_TABLEPREFIX . "stats_links sl, " . SENDSTUDIO_TABLEPREFIX . "links l ";

		if ($listid > 0) {
			$query .= " INNER JOIN " . SENDSTUDIO_TABLEPREFIX . "list_subscribers ls ON (sl.subscriberid = ls.subscriberid)";
		}

		$query .= " WHERE";
		if ($listid > 0) {
			$query .= " ls.listid = $listid AND sl.statid IN(" . implode(',', $statids) . ") ";
		} else {
			$query .= " sl.statid IN(" . implode(',', $statids) . ") ";
		}

		$query .= " AND sl.linkid=l.linkid GROUP BY url, l.linkid";
	...
	*/
	...
	/**
	* ReCheckUserStats
	* Checks whether this user is allowed to send these emails or not. This is used when the size of the send queue has changed.
	*
	* @param Object $user_object User Object to check.
	* @param Int $original_queuesize The size of the queue when the send was originall scheduled
	...
	function ReCheckUserStats(&$user_object, $original_queuesize=0, $new_queuesize=0, $queuetime=0, $statid=0)
	{
		...
		*/
		...
		if (($per_month > 0) && ($new_queuesize > $per_month)) {
			...
			$warning_limit = $new_queuesize - $per_month;
			...
			if ($existing_queuesize >= $per_month) {
				$this->NotifyAdmin($userid, $warning_limit, $queuetime, 'OverLimit_PerMonth');
	...
	function CheckUserStats(&$user_object, $queuesize=0, $queuetime=0, $only_check_permonth=false)
	{
		...
		*/
		...
		if (($per_month > 0) && ($new_queuesize > $per_month)) {
			...
			$warning_limit = ($existing_queuesize + $queuesize) - $per_month;
	...
	*/
	function CalculateStatsType($idx=false)
	{
		...
		if ($idx == true) {
			$calendar_settings = $session->Get('IndexCalendar');
		}
	...
	*/
	function SummarizeEmailSend($queueid=0, $statid=0, $userid=0)
	{
		 ...
		 */
			$rs = $this->Db->Query(	'SELECT htmlrecipients, textrecipients, multipartrecipients, starttime, finishtime'.
									' FROM ' . SENDSTUDIO_TABLEPREFIX . 'stats_newsletters'.
		 ...
		 */
		$query = 'SELECT SUM(emailssent) as count FROM ' . SENDSTUDIO_TABLEPREFIX . "user_stats_emailsperhour WHERE userid = $mUserID AND statid = $mStatID";
		$result = $this->Db->Query($query);
		 ...
		 */
			if ($totalRecipient > 0) {
				...
				for ($i = 0; $i <= $tempHours; ++$i) {
					...
					$status = $this->Db->Query(	'INSERT INTO ' . SENDSTUDIO_TABLEPREFIX . 'user_stats_emailsperhour(statid, sendtime, emailssent, userid)'.
	...
	*/
	...
	function GetMostRecent($count = 1)
	{
		$user = &GetUser();
		$query = "SELECT statid FROM " . SENDSTUDIO_TABLEPREFIX . "stats_users WHERE userid=$user->userid AND statid!=0 ORDER BY queuetime DESC LIMIT " . $count; 
		return $this->Db->FetchOne($query);
	}
}
