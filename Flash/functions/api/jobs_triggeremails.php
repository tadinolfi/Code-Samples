...
class Jobs_TriggerEmails_API extends TriggerEmails_API
{
	...
	private function _send($triggerrecord, $queuerecord, $newsletter)
	{
		 ...
		 * Set up recipient
		 */
			$subscriberinfo = $subscriberAPI->LoadSubscriberList($recipientid, false, true, false, true);
			...
			$status = $email->Send(true, true, true);
			if (!$status['success']) {
					...
					FROM    -- Select All of the required trigger emails information
					        (   SELECT  t.triggeremailsid       AS triggeremailsid,
					            ...
					            JOIN [|PREFIX|]subscribers_data AS d
					                ON (    d.subscriberid = s.subscriberid
					                        AND d.fieldid = t.customfieldid
					                        AND CAST(substr(d.data, 7, 4) AS INTEGER) <= {$year}
					                        AND (  (    t.triggerinterval <> 0
					                                    AND CAST(('{$year}-' || substr(d.data, 4, 2) || '-' || substr(d.data, 1, 2)) AS DATE) + CAST(t.triggerhours || ' hour' AS INTERVAL) >= '{$startOfDayString}'
					                                    AND CAST(('{$year}-' || substr(d.data, 4, 2) || '-' || substr(d.data, 1, 2)) AS DATE) + CAST(t.triggerhours || ' hour' AS INTERVAL) <= '{$endOfDayString}')
					                                OR  (   t.triggerinterval = 0
					                                        AND CAST((substr(d.data, 7, 4) || '-' || substr(d.data, 4, 2) || '-' || substr(d.data, 1, 2)) AS DATE) + CAST(t.triggerhours || ' hour' AS INTERVAL) >= '{$startOfDayString}'
					                                        AND CAST((substr(d.data, 7, 4) || '-' || substr(d.data, 4, 2) || '-' || substr(d.data, 1, 2)) AS DATE) + CAST(t.triggerhours || ' hour' AS INTERVAL) <= '{$endOfDayString}')))
