...
*
* @version     $Id: authentication.php,v 1.11 2008/02/20 07:32:07 chris Exp $
...
class AuthenticationSystem extends API
{
	...
	*/
	 ...
	 */
	static public function GetNewUniqueToken($username)
	{
	 ...
	 *
	 * This method will generate a password hash.
	 * The password hash is then stored in the database instead a plaintext password.
	 *
	...
	private function _authenticate($username, $password, $xmltoken)
	{
		...
		$query = "SELECT * FROM {$tablePrefix}users WHERE username = '{$username}' AND status = '1'";
		...
		if (!empty($password)) {
			...
			if (array_key_exists('unique_token', $details)) {
				$tempPassword = $this->GeneratePasswordHash($password, $details['unique_token']);
			} else {
