...
class Segment_API extends API
{
	...
	{
		$query = 'SELECT ' . ($countonly? 'COUNT(1) AS count' : '*') . ' FROM ' . SENDSTUDIO_TABLEPREFIX . 'segments';
		...
		if (!is_null($userID)) {
			...
			$subQuery = 'SELECT id FROM ' . SENDSTUDIO_TABLEPREFIX . "user_access WHERE area='segments' AND userid=" . $userID;
	...
	*/
	...
	function _PopulateQueryCache()
	{
		if (!class_exists('Subscribers_API')) {
			require(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'subscribers.php');
		}
