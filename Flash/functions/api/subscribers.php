...
class Subscribers_API extends API
{
	...
	*/
	...
	{
		$query = "SELECT lu.*,l.* FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l LEFT OUTER JOIN " . SENDSTUDIO_TABLEPREFIX . "list_subscribers_unsubscribe lu ON l.subscriberid=lu.subscriberid WHERE l.subscriberid='" . $this->Db->Quote($subscriberid) . "'";
	...
	*/
	function LoadSubscriberCustomFields($subscriberid=0, $listid=0, $customfields=array())
	{
		...
		foreach ($customfields as $pos => $details) {
			...
			$fieldtypes[$details['fieldid']] = array('fieldid' => $details['fieldid'], 'fieldtype' => $details['fieldtype'], 'fieldname' => $details['name'], 'data' => $details['defaultvalue']);
		}
	...
	*/
	...
	/**
	* CheckUnsub
	* This checks whether an email address is already unsbuscribed
	*
	* @param String $email Email address to check.
	*
	* @return Boolean Returns true if the email address has correct syntax, otherwise false.
	*/
	function CheckUnsub($emailaddress)
	{
		$user = &GetUser();
		//Check for Unsubscribes
		$unsubQuery = "SELECT unsubscribed, listid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers WHERE unsubscribed != 0 AND emailaddress='$emailaddress'";
		$unsubResult = $this->Db->Query($unsubQuery);
		if($this->Db->CountResult($unsubResult) > 0)
		{
			// check if belongs to user
			while($unsubRow = $this->Db->Fetch($unsubResult))
			{
				$unsubUserChk = "SELECT ownerid FROM " . SENDSTUDIO_TABLEPREFIX . "lists WHERE ownerid='" . $user->userid . "' AND listid='" . $unsubRow->listid . "'";
				$unsubUserResult = $this->Db->Query($unsubUserChk);
				if($this->Db->CountResult($unsubUserResult) > 0)
				{
					return true;
				}
			}
	...
	/**
	* CheckBounce
	* This checks whether an email address has already hardbounced
	*
	* @param String $email Email address to check.
	*
	* @return Boolean Returns true if the email address has correct syntax, otherwise false.
	*/
	function CheckBounce($emailaddress, $user=0)
	{
		if ($user == 0)
		{
			return false;
		...
		// Get all ListIDs
		$ListID_qry = "SELECT * FROM " . SENDSTUDIO_TABLEPREFIX . "lists WHERE ownerid='$user'";
		$ListID_result = $this->Db->Query($ListID_qry);
		$ListIDs = array();
		while($ListID_Row = $this->Db->Fetch($ListID_result))
		{
			array_push($ListIDs, $ListID_Row['listid']);
		}
		//Check for Hard Bounces for this address
		$bounceQuery = "SELECT bounced, subscriberid FROM ". SENDSTUDIO_TABLEPREFIX . "list_subscribers WHERE bounced != 0 AND emailaddress='$emailaddress' AND listid IN (" . implode(',', $ListIDs) . ")";
		$bounceResult = $this->Db->Query($bounceQuery);
		...
		if($this->Db->CountResult($bounceResult) > 0)
		{
			while($bounceRow = $this->Db->Fetch($bounceResult))
			{
				$hardBounceChk = "SELECT subscriberid FROM ". SENDSTUDIO_TABLEPREFIX . "list_subscriber_bounces WHERE bouncetype='hard' AND subscriberid='" . $bounceRow['subscriberid'] . "'";
				$hardBounceChkRes = $this->Db->Query($hardBounceChk);
				if ($this->Db->CountResult($hardBounceChkRes) > 0)
				{
					return true;
				}
			}
	...
	*
	* @return Boolean Returns false if an invalid field, subscriber or list is returned, or if it can't save the custom field data. Otherwise returns true.
	*/
	...
	*/
	function AddToList($emailaddress='', $listid=null, $add_to_autoresponders=true, $skip_listcheck=false)
	{
		...
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "lists SET subscribecount=subscribecount + 1 WHERE listid='" . $real_listid . "'";
		$result = $this->Db->Query($query);
	...
	function UpdateEmailAddress($subscriberids=array(), $emailaddress='')
	{
		...
		*/
		...
		foreach ($subscriberids as $p => $subscriberid) {
			...
			$query .= " WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "'";
			$result = $this->Db->Query($query);
		}
		return true;
	}
	...
	*/
	function UpdateList($subscriberid=0, $listid=0)
	{
		...
		$query .= " WHERE listid='" . $this->Db->Quote($listid) . "' AND subscriberid='" . $this->Db->Quote($subscriberid) . "'";
	...
	*/
	function IsUnSubscriber($emailaddress='', $listid=0, $subscriberid=0, $user=0)
	{
		$emailaddress = trim($emailaddress);
		...
		$query = "SELECT subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l"; 
		
		if ($user)
		{	
			$query .= " INNER JOIN ss_lists sl ON l.listid=sl.listid WHERE unsubscribed > 0 AND sl.ownerid='" . (int)$user . "'";
		} else {
	...
	*/
	function DeleteSubscriber($emailaddress='', $listid=0, $subscriberid=0)
	{
		...
		if ($unsub_date > 0) {
			...
			$query2 = "DELETE FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers_unsubscribe WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "'";
			...
			$query2 = "DELETE FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscriber_bounces WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "'";
		} else {
		...
		$query = "DELETE FROM " . SENDSTUDIO_TABLEPREFIX . "subscribers_data WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "'";
		...
		$query = "DELETE FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscriber_events WHERE subscriberid='" . $subscriberid . "'";
		$this->Db->Query($query);
	...
	*/
	function ChangeSubscriberConfirm($status='confirm', $listid=0, $subscriberid=0)
	{
		...
		if ($listid) {
			$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "list_subscribers SET confirmed='" . $this->Db->Quote($status) . "', confirmdate='" . $this->Db->Quote($confirmdate) . "' WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "' AND listid='" . $this->Db->Quote($listid) . "'";
		} else {
			$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "list_subscribers SET confirmed='" . $this->Db->Quote($status) . "', confirmdate='" . $this->Db->Quote($confirmdate) . "' WHERE subscriberid='" . $this->Db->Quote($subscriberid) . "'";
		}
	...
	*/
	function ListConfirm($listid=0, $subscriberid=0)
	{
		...
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX. "list_subscribers SET confirmed='1', confirmip='" . $this->Db->Quote($this->confirmip) . "', confirmdate='" . $confirmdate . "', subscribedate='" . $confirmdate . "' WHERE listid='" . (int)$listid . "' AND subscriberid='" . (int)$subscriberid . "'";
		$result = $this->Db->Query($query);
	...
	*/
	...
	function GetSubscribersFromSegment($segmentIDs, $countonly = false, $queuedetails = null, $sortdetails = array())
	{
		...
		if (empty($queuedetails)) {
			...
			$temp = "SELECT {$queueID}, '{$queueType}', {$queueOwnerID}, subscribers.subscriberid, 0 FROM ";
	...
	*/
	...
	{
		$query = "SELECT bouncetime, bouncetype, bouncerule FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscriber_bounces WHERE subscriberid='" . (int)$subscriberid . "' AND listid='" . (int)$listid . "'";
	...
	*/
	...
	{
		$subscribers = array();
		$query = "SELECT emailaddress FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers ls, " . SENDSTUDIO_TABLEPREFIX . "queues_unsent qu WHERE ls.subscriberid=qu.recipient AND qu.queueid='" .
		$this->Db->Quote($queueid) . "' AND qu.reasoncode='" . $this->Db->Quote($reasoncode) . "'";
		$result = $this->Db->Query($query);
		while ($row = $this->Db->Fetch($result)) {
	...
	*/
	function FetchSubscribersFromSegment($pageid=1, $perpage=20, $segments, $sortdetails=array(), $searchemail = null)
	{
		...
		if (!is_null($searchemail)) {
			$status = $segmentAPI->AppendRule('AND', array('ruleName' => 'email', 'ruleOperator' => 'equalto', 'ruleValues' => array($searchemail)));
			if ($status === false) {
				return array();
			}
		}
					...
					if (is_numeric($eachRule['rules']['ruleName'])) {
						...
						switch ($eachRule['rules']['ruleName']) {
							...
							case 'subscribe':
								...
								$tempTime = array();
								if (count($tempValues) != count($tempOperator)) {
									list($tempDay, $tempMonth, $tempYear) = explode('/', $tempValues[0]);
							...
							case 'campaign':
								if ($eachRule['rules']['ruleOperator'] == 'equalto') {
									if (!array_key_exists('seo', $tables)) {
										$tables['seo'] = ', ' . SENDSTUDIO_TABLEPREFIX . 'stats_emailopens AS seo';
										$tables['sn'] = 'JOIN ' . SENDSTUDIO_TABLEPREFIX . 'stats_newsletters AS sn'
														. ' ON seo.statid = sn.statid';
										array_push($joins, 'subscribers.subscriberid = seo.subscriberid');
									}

									array_push($conditions, 'sn.newsletterid = ' . intval($eachRule['rules']['ruleValues'][0]));
								} else {
									$query =	'SELECT subscriberid FROM ' . SENDSTUDIO_TABLEPREFIX . 'stats_emailopens AS seo'
												. ' INNER JOIN ' . SENDSTUDIO_TABLEPREFIX . 'stats_newsletters AS sn'
	...
	function CheckPermission($userid, $subscribers)
	{
		...
		$query = trim("
			SELECT l.ownerid AS ownerid, ac.userid AS userid
			FROM {$tablePrefix}list_subscribers AS ls
				JOIN {$tablePrefix}lists AS l
					ON (
						ls.listid = l.listid
						AND ls.subscriberid IN ({$implodedSubscribers})
					)
				LEFT JOIN {$tablePrefix}user_access AS ac
					ON (
						l.listid = ac.id
						AND ac.area = 'lists'
		...
		$row_count = 0;
		while ($row = $this->Db->Fetch($result)) {
			if ($row['ownerid'] != $userid && $row['userid'] != $userid) {
				$row_count = 0;
				break;
			}
