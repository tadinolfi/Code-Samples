...
class Jobs_Autoresponders_API extends Jobs_API
{
	...
	*/
	function FetchJob()
	{
		...
		$queues_done_query = "SELECT queueid FROM " . SENDSTUDIO_TABLEPREFIX . "jobs WHERE jobtype='autoresponder' AND lastupdatetime > " . $half_hour_ago;
		$queues_done_query .= " UNION ALL ";
		// ignore autoresponders that are not active.
		$queues_done_query .= " SELECT queueid FROM " . SENDSTUDIO_TABLEPREFIX . "autoresponders WHERE active=0";
		$queues_done_query = "SELECT queueid FROM ({$queues_done_query}) AS x";
