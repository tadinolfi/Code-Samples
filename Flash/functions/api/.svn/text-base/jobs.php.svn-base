...
class Jobs_API extends API
{
	...
	* @param Int $istest Check whether this is a newsletter test T1, T2 etc.
	...
	*/
	function Create($jobtype=null, $when=0, $ownerid=0, $details=array(), $fktype='newsletter', $fkid=0, $lists=array(), $approved=0, $istest=0)
	{
		$ownerid = (int)$ownerid;
		if ($ownerid <= 0) {
			return false;
		...
		$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "jobs (jobtype, jobstatus, jobtime, jobdetails, fktype, fkid, ownerid, approved, istest) VALUES ('" . $this->Db->Quote($jobtype) . "', 'w', " . (int)$when . ", '" . $this->Db->Quote($details) . "', '" . $this->Db->Quote($fktype) . "', " . $fkid . ", " . $ownerid . ", '" . $this->Db->Quote((int)$approved) . "', '" . $istest . "')";
	...
	*/
	function Delete($jobid=0)
	{
		...
		//if this was a test, decrement the test number
		if($job['istest'] == 1 && $job['jobstatus'] != 'c')
		{
			$query = "UPDATE ". SENDSTUDIO_TABLEPREFIX ."newsletters SET currenttest = currenttest -1 WHERE newsletterid = ".$job['fkid'];
			$this->Db->Query($query);
		}
	...
	*/
	function GetJobList($jobtype='send', $queuetype='newsletter', $ids=array(), $countonly=false, $start=0, $perpage=0, $include_unapproved=false, $chronological=true)
	{
	...
	function NotifyOwner($subject=false, $message=false)
	{
		...
		if ($this->jobtype == 'bounce') {
			$query = "SELECT l.listid, l.ownername, l.owneremail
				FROM ". SENDSTUDIO_TABLEPREFIX . "lists l, " . SENDSTUDIO_TABLEPREFIX . "user_permissions p
