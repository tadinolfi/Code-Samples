...
class CustomFields_Number_API extends CustomFields_API
{
	...
	* This returns the string passed in.
	* We don't do any other checks because CheckData has already made sure we don't have any non-numeric characters (apart from dot, comma or space).
	* We can't cast to int in case you have any of those other characters they get removed.
	*
	* @param Mixed $value The value to get the real data from.
	*
	* @see CheckData
	*
	* @return Int Converts the string passed in to an integer before returning it.
	*/
