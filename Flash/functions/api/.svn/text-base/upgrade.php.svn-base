...
class Upgrade_API extends API
{
	...
	*/
	...
	function RunUpgrade($upgrade=false)
	{
		...
		$status =  $session->Get('DatabaseUpgradeStatusList');
	...
	function GetNextUpgrade()
	{
		...
		$upgrades_todo = $session->Get('UpgradesToRun');
		...
		if (empty($versions)) {
			$session->Set('UpgradesToRun', array());
			return null;
		}
		...
		$session->Set('UpgradesToRun', $upgrades_todo);
