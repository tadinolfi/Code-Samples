...
class Bounce_API extends Jobs_API
{
	...
	*/
	function ProcessEmail($emailid=0, $listid=0)
	{
		...
		/**
		 * Not sure why this is here or what it's trying to check against
		 * Comment it out for now just in case we need to put it back in.
		if (
		...
		*/
		...
		foreach ($bounce_emails as $bep => $bounce_email) {
			...
			$this->Subscriber_API->RecordBounceInfo($subscriber_id, $bounce_statid, $bounce_listid, $bounce_type, $bounce_rule, '', $bounce_time);
