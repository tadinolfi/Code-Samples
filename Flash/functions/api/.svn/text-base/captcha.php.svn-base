...
class Captcha_API extends API
{
	...
	*/
	function CreateSecret()
	{
		...
		if ($this->type == 'static') {
			...
			$captcha_counter = (int)$session->Get('CaptchaCounter');
			...
			$session->Set('CaptchaCounter', $captcha_counter);
	...
	*/
	function SetSecret()
	{
		...
		$session->Set('CaptchaCode', $new_code);
	}
	...
	*/
	function ShowCaptcha()
	{
		...
		if ($this->type == 'dynamic') {
			...
			$return .= '<script>
					var url = "' . SENDSTUDIO_RESOURCES_URL . '/form_designs/captcha/index.php?c=" + getRandom(1,1000)';
					if (!$this->inside_sendstudio && !$this->modify_details) {
						$return .= '+ "&ss=" + SetSID()';
					}
					$return .= ';
		...
		for ($i=0; $i < $this->length; $i++) {
			$return .= '
				var url = "' . SENDSTUDIO_RESOURCES_URL . '/form_designs/captcha/index.php?c=' . $i . '&r=" + getRandom(1,1000)';
				if (!$this->inside_sendstudio && !$this->modify_details) {
					$return .= '+ "&ss=" + SetSID()';
				}
				$return .= ';
					document.write("<img src=\'" + url + "\' alt=\'img' . $i . '\'/>");
				';
		}
		$return .= "</script>\n";
		return $return;
