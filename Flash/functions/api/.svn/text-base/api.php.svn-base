...
class API
{
	...
	function RemoveDuplicatesInQueue($queueid=0, $queuetype='send', $listids=array())
	{
		...
		*/
		if (SENDSTUDIO_DATABASE_TYPE == 'pgsql') {
			...
			if (!empty($listids)) {
				$select_distinct_query .= "	AND l.listid IN (" . implode(',', $listids) . ")";
			}
		...
		*/
		while (true) {
			$query = "SELECT l.subscriberid AS subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers l";
			$query .= " , " . SENDSTUDIO_TABLEPREFIX . "queues q WHERE q.recipient=l.subscriberid AND queueid='" . $this->Db->Quote($queueid) . "' AND queuetype='" . strtolower($queuetype) . "'";

			if (!empty($listids)) {
				$query .= " AND l.listid IN (" . implode(',', $listids) . ")";
			}

			$query .= " GROUP BY emailaddress HAVING COUNT(emailaddress) > 1";
			...
			/**
			* If we find duplicates, remove them from the queue.
			*/
	...
	*/
	...
	{
		if (!$override_settings && !SENDSTUDIO_IPTRACKING) {
			return null;
		}
