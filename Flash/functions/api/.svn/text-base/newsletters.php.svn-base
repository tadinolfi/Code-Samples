...
class Newsletters_API extends API
{
	...
	*/
	...
	/**
	* The html version of the header
	*
	* @var String
	*/
	var $htmlheader = '';
	...
	/**
	* The html version of the footer
	*
	* @var String
	*/
	var $htmlfooter = '';
	...
	/** The current test number of the newsletter 
	*
	* @var Int
	*/
	var $currenttest = 1;
	...
	*/
	function Load($newsletterid=0, $bodyOnly = false)
	{
		...
		$this->currenttest = $newsletter['currenttest'];
	...
	*/
	function GetLastSent($newsletterid=0)
	{
		...
		$newsletterid = (int)$newsletterid;
		$query = "SELECT jobid, starttime, finishtime, htmlrecipients + textrecipients + multipartrecipients AS total_recipients, sendsize FROM " . SENDSTUDIO_TABLEPREFIX . "stats_newsletters WHERE newsletterid='" . $this->Db->Quote($newsletterid) . "' AND sendtype='newsletter' ORDER BY starttime DESC LIMIT 1";
		$result = $this->Db->Query($query);
	...
	*/
	function GetNewsletters($ownerid=0, $sortinfo=array(), $countonly=false, $start=0, $perpage=10)
	{
		...
		$query = "SELECT n.*, u.username AS username, u.fullname AS fullname, ";
		$query .= "CASE WHEN u.fullname = '' THEN u.username ELSE u.fullname END AS owner ";
		$query .= "FROM [|PREFIX|]newsletters n LEFT JOIN [|PREFIX|]users u ON n.ownerid = u.userid ";
	...
	*/
	...
	/**
	* ParseHtmlBody
	* This function parses the header and footer from the html body
	*
	* @return void
	*/
	...
	{
		if(strpos($this->htmlbody,chr(10).'<!--Section-->'.chr(10))){
			$bodySections = explode(chr(10).'<!--Section-->'.chr(10),$this->htmlbody);
			if(count($bodySections)  == 3){
				$this->htmlheader = $bodySections[1];
				$this->htmlbody = $bodySections[0].$bodySections[2].$bodySections[3];
				$this->htmlfooter = '';
			}else{
				$this->htmlheader = $bodySections[1];
				$this->htmlbody = $bodySections[0].$bodySections[2].$bodySections[4];
				$this->htmlfooter = $bodySections[3];
			}
		}else{
			$this->htmlheader = '';
			$this->htmlfooter = '';
	...
	*/
	...
	{
		if(strpos($this->textbody,chr(10).'-----'.chr(10))){
			$bodySections = explode(chr(10).'-----'.chr(10),$this->textbody);
			$this->textheader = $bodySections[0];
			$this->textbody = $bodySections[1];
			$this->textfooter = $bodySections[2];
		}else{
			$this->textheader = '';
			$this->textfooter = '';
		}
	...
	* IncrementTestNumber
	*
	* Increments the test number of this newsletter.
	*
	* @param Int The newsletter to increment
	*/
	function IncrementTestNumber($newsletterid = 0)
	{
		if($newsletterid == 0)
			$newsletterid = $this->newsletterid;
			
		$incQuery = "UPDATE ".SENDSTUDIO_TABLEPREFIX."newsletters SET currenttest = currenttest +1 WHERE newsletterid = $newsletterid";
		$this->Db->Query($incQuery);		
	}
}
