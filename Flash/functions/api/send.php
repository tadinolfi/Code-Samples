...
class Send_API extends Jobs_API
{
	...
	*/
	...
	function SetupJob($jobid=0, $queueid=0)
	{
		...
		// This is sloppy, but the whole system is too obfuscated to do this another way
		
		$jobTestQuery = "SELECT fkid, istest FROM ".SENDSTUDIO_TABLEPREFIX."jobs WHERE jobid=$jobid";
		$jobTestRes = $this->Db->Query($jobTestQuery);
		$jobResRow = $this->Db->Fetch($jobTestRes);
		...
		if($jobResRow['istest'] == 1)
		{
			$testnum = $this->Newsletters_API->currenttest;
			$this->newsletter['Subject'] = 'T'.($testnum-1).': '.$this->Newsletters_API->Get('subject');
		} else {
	...
	* @param Int $queueid The queue we're trying to process. This is used to mark them as processed or remove them from the right queue.
	* Passed to Save_Unsent_Recipient, RemoveFromQueue and MarkAsProcessed
	*
	...
	*/
	function SendToRecipient($recipient=0, $queueid=0)
	{
		...
		if (empty($subscriberinfo) || !isset($subscriberinfo['subscribedate'])) {
			$this->Save_Unsent_Recipient($recipient, $queueid, 1, null);
		...
		$disconnect = true;
		if (defined('SENDSTUDIO_CRON_JOB')) {
			$disconnect = false;
		}
		
		require_once 'user.php';
		
		$getUserQuery = "SELECT ownerid FROM ".SENDSTUDIO_TABLEPREFIX."newsletters WHERE newsletterid = ".$this->jobdetails['NewsletterChosen'];
		$getUserRes = $this->Db->Query($getUserQuery);
		$getUserRow = $this->Db->Fetch($getUserRes);
		$user = new User_API($getUserRow['ownerid']);
		...
		$mail_result = $this->Email_API->Send(true, $disconnect, $mta_headers);
	...
	*/
	function NotifyOwner()
	{
		...
		if (!class_exists('SS_Email_API')) {
			require SENDSTUDIO_API_DIRECTORY . DIRECTORY_SEPARATOR . 'ss_email.php';
		}

		$notify_email = new SS_Email_API;

		// If SMTP is specified, use it.... Notification will always use the global settings.
