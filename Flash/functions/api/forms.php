...
class Forms_API extends API
{
	...
	*/
	...
	/**
	 * Form Hash for added security of external forms
	 * 
	 */
	var $formhash = '';
	...
	* @see Load
	*
	* @return Boolean Returns true if there is no formid passed in. If there is a formid passed in, it will return the value from Load.
	*/
	...
	* @see LoadCustomFields
	*
	* @return Boolean Will return false if the formid is not present, or the form can't be found, otherwise it set the class vars and return true.
	*/
	function Load($formid=0)
	{
		...
		$query = "SELECT * FROM " . SENDSTUDIO_TABLEPREFIX . "forms WHERE formid='" . $formid . "'";
		...
		$this->requireconfirm = trim($form['requireconfirm']);
	...
	/**
	 * LoadWithHash
	 * Loads an external form with hash checking for an extra layer of security
	 * 
	 * @param String $hash an MD5 hash
	 * 
	 * @return Will return false if the formid is not present, or the form can't be found, otherwise it set the class vars and return true.
	 */
	
	 function LoadWithHash($formid=0, $hash=null)
	 {
		$formid = (int)$formid;
		
		if ($formid <= 0 || is_null($hash)) {
			return false;
		}
		

		$query = "SELECT * FROM " . SENDSTUDIO_TABLEPREFIX . "forms WHERE formid='" . $formid . "' AND formhash='" . $this->Db->Quote($hash) . "'";
		$result = $this->Db->Query($query);
		if (!$result) {
			return false;
		}
	...
	* @see pages
	*
	* @return Boolean Returns false if the pages can't be loaded, otherwise the variables are set and this returns true.
	*/
	...
	*/
	function Create()
	{
		...
		$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "forms(name, design, chooseformat, changeformat, contactform, usecaptcha, sendthanks, requireconfirm, formtype, formhtml, ownerid, createdate, formhash) VALUES ('" .  $this->Db->Quote($this->name) . "', '" . $this->Db->Quote($this->design) . "', '" . $this->Db->Quote($this->chooseformat) . "', '" . $this->Db->Quote((int)$this->changeformat) . "', '" . $contactform . "', '" . $this->Db->Quote((int)$this->usecaptcha) . "', '" . $this->Db->Quote((int)$this->sendthanks) . "', '" . $this->Db->Quote((int)$this->requireconfirm) . "', '" . $this->Db->Quote($this->formtype) . "', '" . $this->Db->Quote($this->formhtml) . "', '" . $this->Db->Quote($this->ownerid) . "', '" . $this->GetServerTime() . "', '" . $this->Db->Quote($this->formhash) . "')";
	...
	*/
	function Save()
	{
		...
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "forms SET name='" . $this->Db->Quote($this->name) . "', design='" . $this->Db->Quote($this->design) . "', chooseformat='" . $this->Db->Quote($this->chooseformat)
		. "', changeformat='" . $this->Db->Quote((int)$this->changeformat) . "', sendthanks='" . $this->Db->Quote((int)$this->sendthanks) . "', requireconfirm='" . $this->Db->Quote((int)$this->requireconfirm) . "',
		contactform='" . $contactform . "', usecaptcha='" . $this->Db->Quote((int)$this->usecaptcha) . "', formtype='" . $this->Db->Quote($this->formtype) . "', formhtml='" . $this->Db->Quote($this->formhtml) . "' WHERE formid='" . $this->Db->Quote($this->formid) . "'";
	...
	function SaveCustomFields($formid=0)
	{
		...
		if (!empty($this->customfields)) {
			foreach ($this->customfields as $t => $fieldid) {
				...
				$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "form_customfields(formid, fieldid, fieldorder) VALUES ('" . $formid . "', '" . $this->Db->Quote((int)$fieldid) . "', '" . $this->Db->Quote($fieldorder) . "')";
				$this->Db->Query($query);
			}
		}

		$fieldorder = array_search('e', $this->fieldorder) + 1;
		$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "form_customfields(formid, fieldid, fieldorder) VALUES ('" . $formid . "', 'e', '" . $this->Db->Quote($fieldorder) . "')";
		$this->Db->Query($query);

		$fieldorder = array_search('cl', $this->fieldorder);
		if ($fieldorder !== false && !is_null($fieldorder)) {
			$fieldorder++;
			$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "form_customfields(formid, fieldid, fieldorder) VALUES ('" . $formid . "', 'cl', '" . $this->Db->Quote($fieldorder) . "')";
			$this->Db->Query($query);
		}

		$fieldorder = array_search('cf', $this->fieldorder);
		if ($fieldorder !== false && !is_null($fieldorder)) {
			$fieldorder++;
			$query = "INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "form_customfields(formid, fieldid, fieldorder) VALUES ('" . $formid . "', 'cf', '" . $this->Db->Quote($fieldorder) . "')";
			$this->Db->Query($query);
		}
		return true;
	}
	...
	*/
	function GetHTML($inside_sendstudio=false)
	{
		...
		*/
		...
		if (!class_exists('CustomFields_API')) {
			require(dirname(__FILE__) . '/customfields.php');
		}
		...
		foreach ($displayfields as $p => $field) {
			...
			$option = $subfield->DisplayFieldOptions(false, true, $this->formid);
				...
				switch ($subfield->fieldtype) {
					...
					case 'date':
						foreach (array('dd', 'mm', 'yy') as $p => $datepart) {
							...
							$text = preg_match('%'.$match_string.'%i', $option, $matches);
