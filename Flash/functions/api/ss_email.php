...
class SS_Email_API extends Email_API
{
	...
	*/
	function _GetLinks()
	{
		...
		foreach ($link_locations as $tlinkid => $url) {
			...
			*/
			if (preg_match('~^href\s*?=\s*?["|\'](http://%|%).+(%|%/)["|\']$~i', $url)) {
				continue;
			}
	...
	*/
	function GetCustomFields()
	{
		...
		$built_in_fields = array('emailaddress', 'unsubscribelink', 'confirmlink', 'openimage', 'mailinglistarchive', 'webversion', 'companyname', 'companyphone', 'companyaddress');
	...
	*/
	function _ReplaceCustomFields(&$text, &$subscriberaddress, $header = false)
	{
		...
		$open_image = '<img src="' . SENDSTUDIO_APPLICATION_URL . '/open.php?' . str_replace('&C='.$info['confirmcode'], '', $linkdata) . '&F=H">';
		...
		if (isset($info['CustomFields'])) {
			...
			foreach ($customfields as $p => $details) {
				...
				$replacetext = str_replace(array('\\','$'),array('\\\\','\\$'),$replacetext);
	...
	*/
	function Send($replace=false, $disconnect_from_smtp=true, $add_headers = false)
	{
		...
		if ((int)$this->statid > 0) {
			...
			$extra_headers[] = 'X-Mailer-Sending-System: 7';
		...
		if($add_headers)
		{
			foreach($add_headers as $addheader)
			{
				$extra_headers[] = $addheader;
			}
		}
