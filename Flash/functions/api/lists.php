...
class Lists_API extends API
{
	...
	function Create()
	{
		...
		if ($result) {
			...
			foreach ($this->customfields as $fieldid) {
				$this->Db->Query("INSERT INTO " . SENDSTUDIO_TABLEPREFIX . "customfield_lists (listid, fieldid) VALUES ('" . $listid . "', '" . $this->Db->Quote($fieldid) . "')");
			}
	...
	*/
	function Delete($listid=0, $userid=0)
	{
		...
		$query = "DELETE FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscriber_events WHERE listid=" . $listid;
		$result = $this->Db->Query($query);
	...
	*/
	function DeleteAllSubscribers($listid=0, $called_inside_lists=false)
	{
		 ...
		 */
			$querySubscriberInList = 'SELECT subscriberid FROM ' . SENDSTUDIO_TABLEPREFIX . 'list_subscribers WHERE listid=' . intval($listid);
			$query = 'DELETE FROM ' . SENDSTUDIO_TABLEPREFIX . 'subscribers_data WHERE subscriberid IN (%%TPL_SUBSCRIBERIDS%%)';
			if ($this->_subqueryCapable()) {
				$query = str_replace('%%TPL_SUBSCRIBERIDS%%', $querySubscriberInList, $query);
				$result = $this->Db->Query($query);
			} else {
				$status = $this->Db->Query($querySubscriberInList);
				if ($status !== false) {
					$subscribers = array();
					while (($row = $this->Db->Fetch($status)) !== false) {
						array_push($subscribers, $row['subscriberid']);
					}

					$this->Db->FreeResult($status);

					$tempQuery = str_replace('%%TPL_SUBSCRIBERIDS%%', implode(',', $subscribers), $query);
	...
	*/
	function MergeLists($lists_to_merge = array(), $userinfo=array())
	{
		...
		*/
		...
		$query = "SELECT emailaddress, bounced FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers WHERE listid IN (" . implode(',', $lists_to_merge) . ") AND bounced <> 0 GROUP BY emailaddress";
		$result = $this->Db->Query($query);
		while ($row = $this->Db->Fetch($result)) {
			$new_subscriberid_query = "SELECT subscriberid FROM " . SENDSTUDIO_TABLEPREFIX . "list_subscribers WHERE emailaddress='" . $this->Db->Quote($row['emailaddress']) . "' AND listid=" . $newid;
			$new_subscriber_result = $this->Db->Query($new_subscriberid_query);
	...
	*/
	 ...
	 */
	function GetListByUserID($userid = 0, $getUnconfirmedCount = false)
	{
		...
		if (!array_key_exists($key, $GLOBALS['Lists_API::GetListByUserID[listCache]'])) {
			 ...
			 * Add in "list" table
			 */
				array_push($tempSelects, 'list.*');
				$tempTables['list'] = "{$tablePrefix}lists AS list";
				if ($userid != 0) {
					$tempTables['list'] .= ' ' . trim("
						JOIN {$tablePrefix}user_access AS access
							ON (
								list.listid=access.id
								AND access.area='lists'
			 ...
			 */
				array_push($tempSelects, 'autoresponder.autorespondercount');
				$tempTables['list'] .= ' ' . trim("
					LEFT JOIN (
						SELECT a.listid, COUNT(a.listid) AS autorespondercount
						FROM {$tablePrefix}autoresponders AS a
						GROUP BY a.listid
					) AS autoresponder
			...
			/**
			 * If we need to get unconfirmed subscriber count, we also need to join with list_subscribers table
			 */
				if ($getUnconfirmedCount) {
					array_push($tempSelects, 'subscribers.unconfirmedsubscribercount');
					$tempTables['list'] .= ' ' . trim("
						LEFT JOIN (
							SELECT listid, COUNT(1) AS unconfirmedsubscribercount
							FROM {$tablePrefix}list_subscribers
							WHERE
								confirmed <> '1'
								AND bounced = 0
			...
			$tempQuery = 'SELECT ' . implode(', ', $tempSelects) . ' FROM ' . implode(', ', $tempTables) . ' ORDER BY LOWER(list.name) ASC';
