		...
		if (isset($_GET['Action'])) {
			...
			switch ($action) {
				...
				case 'popupiframe':
					...
					$template = file_get_contents(SENDSTUDIO_TEMPLATE_DIRECTORY . '/progresreport_popup.tpl');
	...
	*/
	...
	function ShowStep_4()
	{
		...
		if (!class_exists('MySQLDb')) {
			require(SENDSTUDIO_LIB_DIRECTORY . '/database/mysql.php');
		}
		$db_type = 'MySQLDb';
		$db = new $db_type();
	...
	function UpgradeDatabase()
	{
		...
		if (!class_exists(SENDSTUDIO_DATABASE_TYPE . 'Db')) {
		...
		if (!$upgrades_done) {
			...
			$session->Set('UpgradeCount', $upgrades_to_run['number_to_run']);
		}
		...
		if (!is_null($running_upgrade)) {
			// ----- Upgrade message
				...
				$statusList = $session->Get('DatabaseUpgradeStatusList');
			...
			$totalUpgradeCount = $session->Get('UpgradeCount');
