...
class Settings extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		switch ($action) {
			case 'addons':
				...
				try {
					...
					if (!in_array(strtolower($subaction), $allowed_sub_action)) {
						FlashMessage(GetLang('Addon_Action_NotAllowed'), SS_FLASH_MSG_ERROR, 'index.php?Page=Settings&Tab=5');
						return;
					}

					$result = Interspire_Addons::Process($_GET['Addon'], $subaction, $post);
					if ($result === true) {
						FlashMessage(GetLang('Addon_Success_' . strtolower($_GET['SubAction'])), SS_FLASH_MSG_SUCCESS, 'index.php?Page=Settings&Tab=5');
						return;
					}
					if ($result === false || $result == null) {
						FlashMessage(GetLang('Addon_Failure_' . strtolower($_GET['SubAction'])), SS_FLASH_MSG_ERROR, 'index.php?Page=Settings&Tab=5');
						return;
					}
					echo $result;
				} catch (Exception $e) {
					$error = $e->GetMessage();
					FlashMessage($error, SS_FLASH_MSG_ERROR, 'index.php?Page=Settings&Tab=5');
				}
				return;
			break;
			...
			case 'viewdisabled':
				...
				switch ($reporttype) {
					case 'autoresponder':
						...
						$disabled_list = $session->Get('AutorespondersDisabled');
					...
					case 'newsletter':
						...
						$disabled_list = $session->Get('NewslettersDisabled');
			...
			case 'save':
				...
				if ($result) {
					FlashMessage(GetLang('SettingsSaved'), SS_FLASH_MSG_SUCCESS, 'index.php?Page=Settings');
				} else {
					FlashMessage(GetLang('SettingsNotSaved'), SS_FLASH_MSG_ERROR, 'index.php?Page=Settings');
				}
			break;
	...
	function ShowSettingsPage()
	{
		...
		//list($license_error, $msg) = ssQmz44Rtt();
		$license_error = '';
		$msg = '';
		$extra = '';
		...
		foreach ($cron_options as $p => $cron_option) {
			...
			if ($last_run > 0) {
				$GLOBALS['Cron_LastRun'] = $this->PrintDate($last_run, GetLang('Cron_DateFormat'));
				if ($settings_var > 0) {
					$next_run = $last_run + ($settings_var * 60);
					$GLOBALS['Cron_NextRun'] = $this->PrintDate($next_run, GetLang('Cron_DateFormat'));
				} else {
		...
		if (!$allow_attachments) {
			...
			if (!empty($autos_to_disable)) {
				...
				if (!empty($disabled_list)) {
					...
					$session->Set('AutorespondersDisabled', $disabled_list);
				}
			}
			...
			if (!empty($newsletterts_to_disable)) {
				...
				if (!empty($disabled_list)) {
					...
					$session->Set('NewslettersDisabled', $disabled_list);
				}
			}
		}
	...
	*/
	...
	function TestBounceSettingsDisplay()
	{
		...
		$test_bounce_details = array (
			'server' => urldecode($_GET['Bounce_Server']),
			'username' => urldecode($_GET['Bounce_Username']),
			'password' => base64_encode(urldecode($_GET['Bounce_Password'])),
			'extra_settings' => urldecode($_GET['Bounce_ExtraSettings']),
			'imap' => (isset($_GET['Bounce_Imap'])) ? 1 : 0,
		);
		$session->Set('TestBounceDetails', $test_bounce_details);
	...
	function TestBounceSettings()
	{
		...
		$test_bounce_details = $session->Get('TestBounceDetails');
