...
class Templates extends SendStudio_Functions
{
	...
	function Process()
	{
			...
			if ($id == 0 && $action == 'create') {
				...
				if (in_array($id, $templates)) {
					...
					if ($effective_permission[$action] == 'view') {
						...
						$access = $user->HasAccess('Templates', $effective_permission[$action], $id);
					}
				}
			}
		}
		...
		switch ($action) {
			case 'viewcompatibility':
				$session = &GetSession();
				$template_info = $session->Get('Templates');
			...
			case 'edit':
				...
				switch ($subaction) {
					case 'step2':
						...
						$session->Set('Templates', $edittemplate);
						$this->DisplayEditTemplate($id);
					break;

					case 'save':
					case 'complete':
						$session_template = $session->Get('Templates');
			...
			case 'create':
				...
				switch ($subaction) {
					case 'step2':
						...
						$session->Set('Templates', $newtemplate);
	...
	*/
	function ManageTemplates()
	{
		...
		foreach ($mytemplates as $pos => $templatedetails) {
			...
			if ($user->HasAccess('Templates', 'Approve', $templateid)) {
			...
			if ($user->HasAccess('Templates', 'Global', $templateid)) {
			...
			if ($user->HasAccess('Templates', 'Edit', $templateid)) {
			...
			if ($user->HasAccess('Templates', 'Delete', $templateid)) {
	...
	*/
	function DisplayEditTemplate($templateid=0, $server_template=false)
	{
		...
		$session->Set('Templates', $session_template);
