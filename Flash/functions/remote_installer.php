...
class Remote_Installer extends SendStudio_Functions
{
	...
	function Remote_Installer()
	{
		...
		*/
		...
		if (!isset($_POST['xml'])) {
			...
			if (empty($raw_input)) {
				$this->Error('The remote installer could not process the request',array(array('code' => 'initError', 'message' => 'No data hase been given to the XML-API.')));
			}
			$_POST['xml'] = $raw_input;
		}
	...
	*/
	function Install()
	{
		...
		list($error, $msg) = ssQmz44Rtt($license_key);
		if ($error) {
		...
		*/
		...
		require(dirname(__FILE__) . "/lib/database/mysql.php");
		$db_type = 'MySQLDb';
		$db_type_name = 'mysql';
		...
		*/
		...
		// ----- Update the default user account
			$unique_token = AuthenticationSystem::GetNewUniqueToken('admin');
			$new_password = AuthenticationSystem::GeneratePasswordHash($install->user->password, $unique_token);

			$query = "
				UPDATE {$tableprefix}users
				SET	usertimezone='" . $db->Quote($settings_details['SERVERTIMEZONE']) . "',
					emailaddress='" .  $db->Quote($settings_details['EMAIL_ADDRESS']) . "',
					textfooter='" . $db->Quote(LNG_Default_Global_Text_Footer) . "',
					htmlfooter='" . $db->Quote(LNG_Default_Global_HTML_Footer) . "',
					unique_token='" . $db->Quote($unique_token) . "',
					password='" . $db->Quote($new_password) . "',
				WHERE userid = 1
			";
		...
		// ----- Update the default user account
			$unique_token = $authentication_api->GetNewUniqueToken('admin');
			$new_password = $authentication_api->GeneratePasswordHash($install->user->password, $unique_token);
			...
			$query = "
				UPDATE {$tableprefix}users
				SET usertimezone= '{$tempServerTimeZone}',
