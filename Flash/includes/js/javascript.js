...
/**
 * Ui.showVerticalInfoBox
 * Show an Information box appropriate for items in a vertically displayed list
 * The inner HTML box can be justifued left, right or centre in relation to the controlling element
 *
 * @param Integer divId the id for the element this inner HTML box is attached to
 * @param String heading a heading for this inner HTML box box
 * @param String text the text dispalyed in the inner HTML box
 * @param String justify is the inner HTML aligned 'left | centre | right' in relation to the controlling element ?
 * @param Integer width a width for the display
 */
Application.Ui.showVerticalInfoBox = function(divId, heading, text, justify, width) {
	var margin = 0;
	if (justify == 'centre') {
		margin = ((width/2) + 5);
	}
	...
	var innerHTML = '<div style="margin-left:-' + margin + 'px; background-color:#FEFCD5; border:solid 1px #E7E3BE; padding:10px;">';
	innerHTML += '<span class=helpTip><b>' + heading + '</b></span>' +
				'<br/><span style="display:inline; padding-left:10; padding-right:5">' + text + '</span></div>';
 ...
 */
 ...
 /**
 * Ui.hideDivElement
 * remove a div element from display
 */
Application.Ui.hideDivElement = function(divId) {
	div = document.getElementById(divId);
...
/**
 * Ui.Menu.PopDown
 * Defines PopDown menu implementation
 */
Application.Ui.Menu._.PopDown = {
	...
	eventMenuClick: function(event) {
		...
		menu.css({	'position': 'absolute',
					'top': offsetTop+this.offsetHeight+1+"px",
...
Application.Ui.Menu.PopDown = function(selector, params) {
...
Application.Ui.CheckboxSelection = function(allSelector, eachSelector, params) {
	var defaultParams = {
	...
	$(allSelector).bind('click', defaultParams, Application.Ui._.CheckboxSelection.eventClickAll);
	$(eachSelector).bind('click', defaultParams, Application.Ui._.CheckboxSelection.eventClickEach)
}
/**
 * -----
 */
...
Application.Ui.Table = {
	...
	GridSetup: function() {
		$('tr.GridRow').hover(Application.Ui.Table.eventGridRowHover, Application.Ui.Table.eventGridRowHover);
	}

}

Application.init.push(Application.Ui.Table.GridSetup);
...
function LaunchHelp(articleid) {
	var id = 'NaN';
	if (Application.Util.isDefined(articleid)) id = parseInt(articleid);
	var help_win = window.open("http://help.sendlabs.com", "help", "width=1024, height=768, left="+(screen.availWidth-700)+", top=100");
}

/* Tab menus */
$(document).ready(function() {
	$('#headerMenu ul .dropdown:last ul').css('margin-left', '-163px'); //I adjust the Reports menu so it shifts over a bit since it gets cut off on the edge. 3/23/09 John Norton
	$('#headerMenu ul .dropdown').bind('mouseover', function(){$(this).addClass('mOverMe');});
	$('#headerMenu ul .dropdown').bind('mouseout', function(){$(this).removeClass('mOverMe');});
/*	$('#headerMenu ul li.dropdown > a').dblclick(function(e)
	...
	$('#headerMenu ul li.dropdown ul').hover(function(e){this.parentNode.overmenu = true;},function(e) 
	{ 
		//alert(this.overmenu + "\n" + this.parentNode.overmenu + "\n" + this.parentNode.parentNode.overmenu);
		if(this.parentNode.overmenu != true)
		{
			$(this.parentNode).removeClass('over');
			$(this.parentNode).find('ul').css('display', 'none');
			$('embed, object, select').css('visibility', 'visible');
		}
		if(this.overmenu != true)
		{
			$(this).removeClass('over');
			$(this).find('ul').css('display', 'none');
			$('embed, object, select').css('visibility', 'visible');
		}
		if(this.parentNode.parentNode.overmenu != true)
		{
			$(this.parentNode.parentNode).removeClass('over');
	...
	$('#headerMenu ul li.dropdown > a').hover(function(e)
	{
		var elem = this;
		...
		if(offsetLeft + menuWidth > $(window).width()) {
			...
			$(elem.parentNode).find('ul').css('left',  (offsetLeft-menuWidth+elem.offsetWidth)+'px');
		}
		...
		$(document).one('hover', function(event) {
			if(elem.parentNode.overmenu != true)
			{
				$(elem.parentNode).removeClass('over');
				$(elem.parentNode).find('ul').css('display', 'none');
...
/**
 * Waits until DevEdit has loaded before allowing the form to be
 * submitted, otherwise if we submit too early we might lose our HTML.
 *
 * Assumes the DE var is called 'myDevEditControl'.
 *
 * Use like:
 *
 * var de_ready = false;
...
function waitForDE()
{
	if (typeof(myDevEditControl) == 'undefined') {
		de_ready = true;
		return;
	}
	try {
		myDevEditControl.getHTMLContent();
	} catch (e) {
		if (e instanceof TypeError && myDevEditControl == null) {
			try_again = window.setTimeout(function() { waitForDE(); }, 250);
			return;
		}
		// otherwise it has probably loaded, but in an unsupported browser
	}
	de_ready = true;
}

/**
 * Informs that DevEdit isn't ready yet. Use like:
 *
 * <form onsubmit="return isDEReady('%%LNG_Editor_WaitToLoad%%');">
 *
 * @return bool ready state.
 *
 * @see waitForDE
 */
function isDEReady(msg)
{
	if (!de_ready) {
		alert(msg);
	}
