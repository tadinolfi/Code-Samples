...
<script>
	var PAGE = {
		...
		_addRuleSelectOptions:			function(elm, data) {
			...
			$(	'<optgroup label="Basic Fields (c)">' + selectOptions.basic.join('') + '</optgroup>'
				+ (selectOptions.customfields.length == 0? '' : '<optgroup label="Custom Fields (c)">' + selectOptions.customfields.join('') + '</optgroup>')).appendTo(elm);
		...
		_applyNumberInputboxBehaviour:	function(inputbox) {
			...
			$(inputbox).keypress(function(event){
				if(event.which > 31 && (event.which < 48 || event.which > 57) && event.which != 46 && event.which != 44) {
					event.stopPropagation();
					event.preventDefault();
				}
			});
		...
		_appendDateBetween:				function(row) {
			var temp = $('<span>&nbsp;%%LNG_AND%%&nbsp;<input type="text" readonly="readonly" class="RuleRowUserInput Field250 DateInputType" style="width:125px;" name="RuleValue[' + $(row).attr('rowIndex') + '][]" /></span>');
	...
	<table cellspacing="0" cellpadding="3" width="100%" align="center">
		<tr>
			<td class="Heading1">
                ...
                <div class="Heading1Image"><img src="images/headerimages/create_segment.jpg" alt="%%GLOBAL_Heading%%" /></div><div class="Heading1Text">%%GLOBAL_Heading%%</div><div class="Heading1Links"> <a href="#"><img src="images/help2.gif" alt="Help" border="0" /></a><a href="#"><img src="images/bulb.jpg" alt="Tip" border="0" /></a></div>
			</td>
		</tr>
		<tr>
			<td class="body">
